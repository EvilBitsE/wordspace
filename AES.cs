...
namespace CryptoLib
{
    public class EvEAes  // Advanced Encryption Standard
    {


        private uint[] RCON = new uint[10]
        {
            0x00000001, 0x00000002, 0x00000004, 0x00000008,
            0x00000010, 0x00000020, 0x00000040, 0x00000080,
            0x0000001B, 0x00000036
        };
        private byte[] FSb = new byte[256]
        {
        ...
        private uint[] rk;     // key schedule array. 

        public enum CryptMode { AES_ENCRYPT, AES_DECRYPT };


        public EvEAes()
        {
            this.rk = new uint[68];  // 16, 24, 32 bytes
        }  // Aes constructor

        public int Setkey_enc(byte[] key, KeySize keybits)
        {
            uint keylen = 0;
            switch (keybits)
            {
            ...
            int i = 0;
            int j = 0;
            for (; i < ((int)keylen >> 5); i++)
            {
                rk[i] = GET_UINT32_LE(key, i << 2);
            }
            ...
            switch (Nr)
            {
                case 10:
                    for (i = 0; i < 10; i++, j += 4)
                    {
                        rk[j + 4] = rk[j + 0] ^ RCON[i] ^
                            ((uint)FSb[(rk[j + 3] >> 8) & 0xFF]) ^
                            ((uint)FSb[(rk[j + 3] >> 16) & 0xFF] << 8) ^
                            ((uint)FSb[(rk[j + 3] >> 24) & 0xFF] << 16) ^
                            ((uint)FSb[(rk[j + 3]) & 0xFF] << 24);

                        rk[j + 5] = rk[j + 1] ^ rk[j + 4];
                        rk[j + 6] = rk[j + 2] ^ rk[j + 5];
                        rk[j + 7] = rk[j + 3] ^ rk[j + 6];
                    }
                    break;
                ...
                case 12:
                    ...
                    for (i = 0; i < 8; i++, j += 6)
                    {
                        rk[j + 6] = rk[j + 0] ^ RCON[i] ^
                            ((uint)FSb[(rk[j + 5] >> 8) & 0xFF]) ^
                            ((uint)FSb[(rk[j + 5] >> 16) & 0xFF] << 8) ^
                            ((uint)FSb[(rk[j + 5] >> 24) & 0xFF] << 16) ^
                            ((uint)FSb[(rk[j + 5]) & 0xFF] << 24);

                        rk[j + 7] = rk[j + 1] ^ rk[j + 6];
                        rk[j + 8] = rk[j + 2] ^ rk[j + 7];
                        rk[j + 9] = rk[j + 3] ^ rk[j + 8];
                        rk[j + 10] = rk[j + 4] ^ rk[j + 9];
                        rk[j + 11] = rk[j + 5] ^ rk[j + 10];
                    }
                    break;
                ...
                case 14:
                    ...
                    for (i = 0; i < 7; i++, j += 8)
                    {
                        rk[j + 8] = rk[j + 0] ^ RCON[i] ^
                            ((uint)FSb[(rk[j + 7] >> 8) & 0xFF]) ^
                            ((uint)FSb[(rk[j + 7] >> 16) & 0xFF] << 8) ^
                            ((uint)FSb[(rk[j + 7] >> 24) & 0xFF] << 16) ^
                            ((uint)FSb[(rk[j + 7]) & 0xFF] << 24);
                        ...
                        rk[j + 12] = rk[j + 4] ^
                            ((uint)FSb[(rk[j + 11]) & 0xFF]) ^
                            ((uint)FSb[(rk[j + 11] >> 8) & 0xFF] << 8) ^
                            ((uint)FSb[(rk[j + 11] >> 16) & 0xFF] << 16) ^
                            ((uint)FSb[(rk[j + 11] >> 24) & 0xFF] << 24);
        ...
        public int Setkey_dec(byte[] key, KeySize keybits)
        {           
            if (Setkey_enc(key, keybits) != 0)
                return 2;
            uint[] SK = new uint[68];
            int j = 0;
            int i = Nr * 4;
            ...
            int k = 0;
            int m = 0;
            for (m = Nr - 1,i -= 8 ; m > 0; m--, i -= 8)
            {
                for (k = 0; k < 4; k++, i++)
                {
                    SK[j++] = RT0[FSb[(rk[i]) & 0xFF]] ^
                        RT1[FSb[(rk[i] >> 8) & 0xFF]] ^
                        RT2[FSb[(rk[i] >> 16) & 0xFF]] ^
                        RT3[FSb[(rk[i] >> 24) & 0xFF]];
                }
            }
        ...
        public int Crypt_ecb(CryptMode mode, byte[] input, byte[] output)
        {
            if (mode == CryptMode.AES_ENCRYPT)
                return (internal_aes_encrypt(input, output));
            else
                return (internal_aes_decrypt(input, output));
        }


        public int Crypt_cbc(CryptMode mode, byte[] IV,byte[] input, ref byte[] output)
        {
            int byteLength = input.Length;
            int loop = byteLength / 16;
            int i = 0;
            byte[] bytesp = new byte[16];
            byte[] bytesm = new byte[16];
            byte[] bytest = new byte[16];
            if (mode == CryptMode.AES_DECRYPT)
            {
                while (i < loop)
                {
                    Buffer.BlockCopy(input, i * 16, bytesm, 0, 16);
                    bytesm.CopyTo(bytest, 0);
                    this.Crypt_ecb(mode, bytesm, bytesp);
                    for (int j = 0; j < 16; j++)
                        bytesp[j] = (byte)(bytesp[j] ^ IV[j]);
                    Buffer.BlockCopy(bytesp, 0, output, i * 16, 16);
                    bytest.CopyTo(IV, 0);
                    ++i;
                }
            }
            else
            {
                while (i < loop)
                {
                    Buffer.BlockCopy(input, i * 16, bytesp, 0, 16);
                    for (int j = 0; j < 16; j++)
                        bytesp[j] = (byte)(bytesp[j] ^ IV[j]);                    
                    this.Crypt_ecb(mode, bytesp, bytesm);
                    Buffer.BlockCopy(bytesm, 0, output, i * 16, 16);
                    bytesm.CopyTo(IV, 0);
                    ++i;
                }
            }
        ...
        private uint GET_UINT32_LE(byte[] key,int index)
        {
            return (uint)key[index] + ((uint)key[index + 1] << 8) + ((uint)key[index + 2] << 16) + ((uint)key[index + 3] << 24);
        }
        private void PUT_UINT32_LE(uint x, ref byte[] key, int index)
        {
            key[index] = (byte)(x & 0xFF);
            key[index+1] = (byte)((x >> 8) & 0xFF);
            key[index+2] = (byte)((x >> 16) & 0xFF);
            key[index+3] = (byte)((x >> 24) & 0xFF);
        }
        /*
         * AES-ECB block encryption
         */
        private void AES_FROUND(ref uint X0, ref uint X1, ref uint X2, ref uint X3, uint Y0, uint Y1, uint Y2, uint Y3, ref int i)
        {
            X0 = rk[i++] ^ FT0[(Y0) & 0xFF] ^
                 FT1[(Y1 >> 8) & 0xFF] ^
                 FT2[(Y2 >> 16) & 0xFF] ^
        ...
        private void AES_RROUND(ref uint X0, ref uint X1, ref uint X2, ref uint X3, uint Y0, uint Y1, uint Y2, uint Y3, ref int i)
        {
            X0 = rk[i++] ^ RT0[(Y0) & 0xFF] ^
                 RT1[(Y3 >> 8) & 0xFF] ^
                 RT2[(Y2 >> 16) & 0xFF] ^
        ...
        private int internal_aes_encrypt(byte[] input, byte[] output)
        {
            ...
            Y2 = 0;
            Y3 = 0;
            X0 = GET_UINT32_LE(input, 0); X0 ^= rk[i++];
            X1 = GET_UINT32_LE(input, 4); X1 ^= rk[i++];
            X2 = GET_UINT32_LE(input, 8); X2 ^= rk[i++];
            X3 = GET_UINT32_LE(input, 12); X3 ^= rk[i++];
            for (j = (Nr >> 1) - 1; j > 0; j--)
            {
                AES_FROUND(ref Y0, ref Y1, ref Y2, ref Y3, X0, X1, X2, X3, ref i);
                AES_FROUND(ref X0, ref X1, ref X2, ref X3, Y0, Y1, Y2, Y3, ref i);
            }
            AES_FROUND(ref Y0, ref Y1, ref Y2, ref Y3, X0, X1, X2, X3, ref i);

            X0 = rk[i++] ^ ((uint)FSb[(Y0) & 0xFF]) ^ ((uint)FSb[(Y1 >> 8) & 0xFF] << 8) ^ ((uint)FSb[(Y2 >> 16) & 0xFF] << 16) ^ ((uint)FSb[(Y3 >> 24) & 0xFF] << 24);
            X1 = rk[i++] ^ ((uint)FSb[(Y1) & 0xFF]) ^ ((uint)FSb[(Y2 >> 8) & 0xFF] << 8) ^ ((uint)FSb[(Y3 >> 16) & 0xFF] << 16) ^ ((uint)FSb[(Y0 >> 24) & 0xFF] << 24);
            X2 = rk[i++] ^ ((uint)FSb[(Y2) & 0xFF]) ^ ((uint)FSb[(Y3 >> 8) & 0xFF] << 8) ^ ((uint)FSb[(Y0 >> 16) & 0xFF] << 16) ^ ((uint)FSb[(Y1 >> 24) & 0xFF] << 24);
            X3 = rk[i++] ^ ((uint)FSb[(Y3) & 0xFF]) ^ ((uint)FSb[(Y0 >> 8) & 0xFF] << 8) ^ ((uint)FSb[(Y1 >> 16) & 0xFF] << 16) ^ ((uint)FSb[(Y2 >> 24) & 0xFF] << 24);
            
            PUT_UINT32_LE(X0, ref output, 0);
            PUT_UINT32_LE(X1, ref output, 4);
            PUT_UINT32_LE(X2, ref output, 8);
            PUT_UINT32_LE(X3, ref output, 12);
        ...
        */
        ...
        private int internal_aes_decrypt(byte[] input, byte[] output)
        {
            ...
            for (j = (Nr >> 1) - 1; j> 0; j--)
            {
                AES_RROUND(ref Y0, ref Y1, ref Y2, ref Y3, X0, X1, X2, X3, ref i);
                AES_RROUND(ref X0, ref X1, ref X2, ref X3, Y0, Y1, Y2, Y3, ref i);
            }
            AES_RROUND(ref Y0, ref Y1, ref Y2, ref Y3, X0, X1, X2, X3, ref i);

            X0 = rk[i++] ^ ((uint)RSb[(Y0) & 0xFF]) ^ ((uint)RSb[(Y3 >> 8) & 0xFF] << 8) ^ ((uint)RSb[(Y2 >> 16) & 0xFF] << 16) ^ ((uint)RSb[(Y1 >> 24) & 0xFF] << 24);
            X1 = rk[i++] ^ ((uint)RSb[(Y1) & 0xFF]) ^ ((uint)RSb[(Y0 >> 8) & 0xFF] << 8) ^ ((uint)RSb[(Y3 >> 16) & 0xFF] << 16) ^ ((uint)RSb[(Y2 >> 24) & 0xFF] << 24);
            X2 = rk[i++] ^ ((uint)RSb[(Y2) & 0xFF]) ^ ((uint)RSb[(Y1 >> 8) & 0xFF] << 8) ^ ((uint)RSb[(Y0 >> 16) & 0xFF] << 16) ^ ((uint)RSb[(Y3 >> 24) & 0xFF] << 24);
            X3 = rk[i++] ^ ((uint)RSb[(Y3) & 0xFF]) ^ ((uint)RSb[(Y2 >> 8) & 0xFF] << 8) ^ ((uint)RSb[(Y1 >> 16) & 0xFF] << 16) ^ ((uint)RSb[(Y0 >> 24) & 0xFF] << 24);
